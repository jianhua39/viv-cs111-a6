...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        boolean rainToday = (StdRandom.uniform() < 0.5); // [0,0.5) (wet), [0.5,1) (dry)
        for (int i=0; i<numberOfDays; i++){
            if (rainToday){
                ...
                //result[i] = (StdRandom.uniform() < wetwetProbability) ? WET : DRY;
            }
            else{
                result[i] = (StdRandom.uniform() < drywetProbability) ? WET : DRY;
            }
            rainToday = result[i] == WET;
        }
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for(int i=0; i<forecast.length;i++){
            ...
            if (wetDays < leastWetDays)
            {
                leastWetDays = wetDays;                
                result = i;
            }
            wetDays = 0;
        }
